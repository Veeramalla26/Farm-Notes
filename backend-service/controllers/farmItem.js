...
async function createFarmItem(data, customerId) {
  try {
    ...
    const farmItemExists = await models.FarmItems.findOne({
    ...
    logger.info(`Farm Item created successfully`);
    return farmItem;
  } catch (error) {
    logger.error(error);
    throw error;
...
async function listFarmItems(data, customerId) {
  try {
    ...
    logger.info(`Farm Items fetched successfully`);
    return {
      count: farmItems.rows.length || farmItems.count,
      rows: response,
    };
  } catch (error) {
    logger.error(error);
...
async function editFarmItem(farmItemId, data, customerId) {
  try {
    ...
    logger.info(`Farm Item updated successfully`);
    return {
      farmItemId,
      name: data.name,
    };
...
async function destroyFarmItem(farmItemId) {
  try {
    ...
    if (!farmItemExists) throw new Error("Farm Item not exists");
    await models.FarmItemActivities.destroy({ where: { farmItemId } });
